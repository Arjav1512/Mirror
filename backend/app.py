...
# Load and inject shared styles - CRITICAL FOR DESIGN CONSISTENCY
...
try:
    ...
    # Silently fail if CSS not found - Streamlit has its own styling
    pass

# Additional Streamlit-specific overrides - MATCH LANDING PAGE EXACTLY
st.markdown("""
    <style>
    ...
    /* Text input styling - MATCH LANDING PAGE */
    .stTextArea textarea {
...
def check_auth_token():
    """
    AUTO-LOGIN via JWT token from URL - NO STREAMLIT LOGIN UI EVER
        ...
        if auth_token:
            ...
            if user_data:
                ...
                try:
                    ...
                    pass  # Some Streamlit versions don't support clear()
            ...
            # No token - redirect to landing page immediately
            show_redirect_to_landing()
            return False
...
def show_redirect_to_landing():
    """
    Automatically redirect to landing page - NO LOGIN UI IN STREAMLIT
    """
...
def main_journal_interface():
    """
    Main dashboard - ONLY shown to authenticated users (auto-logged in via JWT)
    """
    # Header - EXACT MATCH to landing page style
    st.markdown(f"""
    ...
    with col_right:
        if st.button("🚪 Sign Out", key="signout", help="Clear session and return to landing page", use_container_width=True):
            # Clear session
    ...
    # Debug info for troubleshooting (uncomment to see)
    ...
    with col2:
        st.markdown("""<h2 style="font-size: 1.5rem; font-weight: 700; color: #f1f5f9; margin-bottom: 1rem;">📊 Emotional Insights</h2>""", unsafe_allow_html=True)
        ...
        if not sentiment_data.empty and 'valence' in sentiment_data.columns and len(sentiment_data) > 0:
            try:
                ...
                st.markdown(f"""
                    <div style="background: linear-gradient(135deg, #3b82f620 0%, #8b5cf610 100%); padding: 1.5rem; border-radius: 0.75rem; margin-bottom: 1rem; border: 1px solid #3b82f630;">
                        <p style="color: #94a3b8; font-size: 0.85rem; margin: 0 0 0.5rem 0;">7-Day Average Sentiment</p>
                        <p style="color: #f1f5f9; font-size: 2rem; font-weight: 700; margin: 0;">{recent_avg:.2f}</p>
                        <p style="color: #cbd5e1; font-size: 0.85rem; margin: 0.5rem 0 0 0;">{'Positive overall trend' if recent_avg > 0 else 'Neutral' if recent_avg == 0 else 'Challenging period'}</p>
                    </div>
                """, unsafe_allow_html=True)
                ...
                volatility_label = "Stable variation (±0.2)" if volatility < 0.3 else "Moderate variation" if volatility < 0.6 else "High variation"
                st.markdown(f"""
                    <div style="background: #1e293b; padding: 1rem; border-radius: 0.75rem; border: 1px solid #334155;">
                        <p style="color: #94a3b8; font-size: 0.85rem; margin: 0 0 0.5rem 0;">Emotional Volatility</p>
            ...
            st.markdown("""
                <div style="background: #1e293b; padding: 2rem; border-radius: 0.75rem; text-align: center; border: 1px solid #334155;">
                    ...
                    <p style="color: #cbd5e1; font-size: 1rem; margin: 0;">Start journaling to see<br>your emotional insights</p>
                </div>
            """, unsafe_allow_html=True)
    ...
    if not sentiment_data.empty and len(sentiment_data) > 0:
        try:
            st.markdown(f"""<p style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 1rem;">Your emotional journey - {len(sentiment_data)} entries tracked</p>""", unsafe_allow_html=True)
    ...
    # Bias Frequency Tracker  
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""<h2 style="font-size: 1.5rem; font-weight: 700; color: #f1f5f9; margin-bottom: 1rem;">🧠 Cognitive Biases Detected</h2>""", unsafe_allow_html=True)
    
    if len(entries) > 0:
        try:
            st.markdown(f"""<p style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 1rem;">Analyzing {len(entries)} entries for cognitive patterns</p>""", unsafe_allow_html=True)
            
            # Get biases for all entries
            all_biases = []
            ...
            if all_biases and len(all_biases) > 0:
                ...
                # Display top biases
                st.markdown(f"""<p style="color: #a78bfa; font-size: 0.9rem; margin-bottom: 1rem;">✓ Found {len(all_biases)} cognitive patterns across your entries</p>""", unsafe_allow_html=True)
                ...
                st.info(f"🧠 No cognitive biases detected yet in your {len(entries)} entries. The AI will identify patterns as you continue journaling!")
        except Exception as e:
            st.error(f"Error analyzing biases: {str(e)}")
            st.info("Unable to analyze cognitive patterns. Please try again.")
    else:
        st.info("📉 Bias tracking will appear after your first entry")


def main():
    """Main entry point - AUTO-LOGIN via JWT or redirect to landing page"""
    if check_auth_token():
        main_journal_interface()
    # If check_auth_token() returns False, it already redirected user
